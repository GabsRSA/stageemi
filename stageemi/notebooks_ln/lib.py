...
def cree_dict_list_agglo(dict_temps, ds_agglo, ds_dep_agglo):
    ...
    for legende_wwme,code_wwme in dict_temps.items():
        ...
        # Boucle sur toutes les agglomerations
        list_agglo = []
        for code_insee in ds_dep_agglo["mask"].id.values:
            ds_it = ds_bin * ds_dep_agglo["mask"].sel(id=str(code_insee))
            taille_objet_binaire = np.sum(ds_it.wme_arr.values==100)
            taille_agglo = np.sum(ds_it.wme_arr.values >-2)
            nom_commune = dict_insee[str(code_insee)]
