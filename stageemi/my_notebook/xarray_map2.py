...
class interactive_map(widg.HBox):
    ...
    def reading_homogeneous_results(self):
        ...
        try:
            ...
            print("No file for Homogeneous criterion")
            zone_id = ["departement"]
    ...
    def get_Mary_zone(self):
        ...
        if hasattr(self,"dfres"):
            self.mary_zone = self.dfres.set_index("zone")['compas']
            print(self.dfres)
        # On lit les resultat des zonages de Mary
...
#            feature_extend = poly_geojson["features"] 
        ...
        #self.toto = dict(zip(self.zone_homo_id, list(np.arange(len(self.zone_homo_id)).astype(float)))) 
        # Pas besoin en theorie. Pourquoi on le fait toujours ? 
        #if hasattr(self,"da"):
        #    self.aggregate()
    ...
    def render(self):
        ...
        if hasattr(self,"dfres"):
            ...
            (code,hexc,_) = geo_gv.get_cmap_info("WME")
        ...
        (code,hexc,_) = geo_gv.get_cmap_info("W1")
        cmap = cm.StepColormap(hexc.values,index=np.asarray(code).astype(float),vmin=code[0],vmax=code[-1]+1)
        print(code[0],code[-1]+1)
